...
color_ranges = {
    'Red': [
        (np.array([0,   25,  80]),  np.array([15,  200, 240])),   # lowered sat min to catch pinkish-red
...
GANTRY_Z_TOP          =   50.0
GANTRY_Z_BOTTOM       =    0.0
CAMERA_VIEW_WIDTH_MM  =  210.0
CAMERA_VIEW_HEIGHT_MM =  250.0
...
# ==============================================================================
# BUILD OBJECT LIST
# FIX: Send actual object_height_mm in the Z field instead of PICK_Z_FIXED,
#      so Arduino can sort same-color objects by height ascending.
# ==============================================================================
def build_object_list(top_results, front_results):
    ...
    for color_name in all_colors:
        ...
        for i in range(max_count):
            ...
            # FIX: send object_height_mm as the Z field so Arduino can sort by height.
            # Arduino ignores this for physical pick Z (it uses its own PICK_Z = 5mm),
            # but reads it into height_mm for ascending sort within same-color groups.
            object_list.append({
                "x"        : gantry_x,
                "y"        : gantry_y,
                "z"        : object_height_mm,  # ← FIX: was PICK_Z_FIXED (45.0)
